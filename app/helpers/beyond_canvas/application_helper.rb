...
module BeyondCanvas
  module ApplicationHelper
    def full_title(page_title = nil)
      base_title = ::BeyondCanvas.site_title

      page_title&.strip.blank? ? base_title : "#{page_title} | #{base_title}"
    end
    ...
    def collapse(name, html_options = nil, &block)
      ...
      html_options.merge!(id:)
      html_options.merge!(data: { 'collapsible-target': 'content' })

      content_tag('div', class: 'collapse', data: { controller: 'collapsible' }) do
        content_tag('a', class: 'collapse__button', title: name,
                         data: { visible: false,
                                 action: 'click->collapsible#toggle', 'collapsible-target': 'headline' }) do
          inline_svg_tag('icons/arrow_right.svg', class: 'collapse__icon',
                                                  data: { 'collapsible-target': 'icon' }) + name
        end +
    ...
    def step_list(title = nil, steps: [])
      content_tag('div', class: 'step-list__container') do
        [
          (content_tag('h4', raw(title), class: 'step-list__title') if title.present?), # rubocop:disable Rails/OutputSafety
              ...
              safe_join(steps.each_with_index.collect do |step, index|
                content_tag('tr') do
                  content_tag('td', class: 'step-list__bubble-column') do
                    ...
                    content_tag('td') do
                      content_tag('strong', raw(step[:headline]), class: 'step-list__headline') + # rubocop:disable Rails/OutputSafety
                        content_tag('p', raw(step[:description]), class: 'step-list__description') # rubocop:disable Rails/OutputSafety
                    end
                end
              end)
            end
    ...
    [:top, :right, :bottom, :left].each do |method|
